...
// from CdiacReader
public class OadsSocatUtils { // extends DocumentHandler {
    ...
    private static final String OADS_EQUI_TYPE_ELEMENT_NAME = EQUILIBRATOR_DESIGN_ELEMENT_NAME + SEP + "Equilibrator_Type";
    ...
    OadsOmeMetadata _oadsMd;
    OadsMetadataDocumentType _oadsDoc;
//    Element _xmlRoot;
    

    public OadsSocatUtils(OadsOmeMetadata oadsMetadata) {
        _oadsMd = oadsMetadata;
        _oadsDoc = _oadsMd.getOadsDocument();
//        _xmlRoot = _oadsMd.getXmlRoot();
        
        keyToTypeMap = new HashMap<String,VarType>(DEFAULT_KEY_TO_TYPE_MAP);
    }
    ...
    private MiscInfo getMiscInfo() {
        ...
        // Dataset ID / Expocode
        String expocode = _oadsMd.getDatasetId(); // getElementText(null, EXPOCODE_ELEMENT_NAME);
//        if ( expocode.isEmpty() )
//            expocode = getElementText(null, CRUISE_ID_ELEMENT_NAME);
        info.setDatasetId(expocode);

        info.setDatasetName(_oadsMd.getDatasetName()); // (null, EXPERIMENT_NAME_ELEMENT_NAME));
        info.setSectionName(getFirst(_oadsDoc.getSections())); // getElementText(null, SECTION_ELEMENT_NAME));

        // Funding information all glummed together in CDIAC XML - stick under agency name
        info.setFundingAgency(getFunding()); // getElementText(null, FUNDING_INFO_ELEMENT_NAME));
...
//        info.setHistory(history);
        ...
        Datestamp datestamp = Datestamp.fromUTC(new Date((long)(_oadsMd.getDataStartTime().doubleValue()*1000)));  // getDatestamp(getElementText(null, START_DATE_ELEMENT_NAME)));
//        if ( datestamp == null ) // XXX TODO Check? OADS doesn't distinguish (currently)
//            datestamp = getDatestamp(getElementText(null, TEMP_START_DATE_ELEMENT_NAME));
        info.setStartDatestamp(datestamp);
        datestamp = Datestamp.fromUTC(new Date((long)(_oadsMd.getDataEndTime().doubleValue()*1000)));  // getDatestamp(getElementText(null, END_DATE_ELEMENT_NAME)));
//        if ( datestamp == null )
...
//        info.setAddnInfo(addnInfo);
     ...
     * @return
     */
    private String getFunding() {
        StringBuilder fundingStr = new StringBuilder();
        List<FundingSourceType> fundings = _oadsDoc.getFundingInfo();
        if ( fundings == null || fundings.isEmpty()) {
            return "";
        }
        String sep = "";
        for ( FundingSourceType funding : fundings ) {
            fundingStr.append(funding.getAgency())
                      .append(" [ ").append(funding.getIdentifier().getValue()).append(" ] ");
            if ( funding.isSetTitle()) {
                fundingStr.append(funding.getTitle());
            }
            fundingStr.append(sep);
            sep = "; ";
        }
        return fundingStr.toString();
    }
     ...
     * @return
     */
    private static String getFirst(List<String> stringList) {
    ...
    private Submitter getSubmitter() {
        PersonType oaSubmitter = _oadsDoc.getDataSubmitter();
        Submitter submitter = new Submitter(getPersonNames(oaSubmitter));
        setPersonInfo(oaSubmitter, submitter);
        return submitter;
    }
     ...
     * @return
     */
    private static Person getPersonNames(PersonType forPerson) {
        Person person = new Person();
        person.setLastName(forPerson.getName().getLast());
        person.setMiddle(forPerson.getName().getMiddle());
        person.setFirstName(forPerson.getName().getFirst());
        return person;
    }

    private static Person setPersonInfo(PersonType forPerson, Investigator asPerson) {
        PersonContactInfoType contactInfo = forPerson.getContactInfo();
        AddressType addr = contactInfo.getAddress();
        List<OrderedStringElementType> oaStreets = addr.getDeliveryPoint();
        ArrayList<String> streets = new ArrayList<>();
        if ( oaStreets  != null ) {
            for (OrderedStringElementType street : oaStreets) {
                String vStreet = street.getValue();
                if ( !vStreet.isEmpty()) {
                    streets.add(vStreet);
                }
            }
        }
        asPerson.setStreets(streets);
        asPerson.setCity(addr.getCity());
        asPerson.setRegion(addr.getAdministrativeArea());
        asPerson.setCountry(addr.getCountry());
        asPerson.setZipCode(addr.getPostalCode());
        asPerson.setOrganization(forPerson.getOrganization());
        asPerson.setPhone(contactInfo.getPhone());
        asPerson.setEmail(contactInfo.getEmail());
        
        List<TypedIdentifierType> ids = forPerson.getIdentifier();
        if ( ids  != null && !ids.isEmpty()) {
            TypedIdentifierType id = ids.get(0);
            asPerson.setId(id.getValue());
            asPerson.setIdType(id.getType());
        }
    ...
    private ArrayList<Investigator> getInvestigators() {
        ...
        for (PersonType oaInvestigator : _oadsDoc.getInvestigators()) {
            Investigator pi = new Investigator(getPersonNames(oaInvestigator));
            setPersonInfo(oaInvestigator, pi);
            piList.add(pi);
        }
        return piList;
    }
    ...
    private Platform getPlatform(String datasetId) {
        ...
        gov.noaa.ncei.oads.xml.v_a0_2_2s.PlatformType oadsPlatform = (_oadsMd.getOadsDocument().getPlatforms().get(0));
        platform.setPlatformId(oadsPlatform.getIdentifier().getValue()); // , VESSEL_ID_ELEMENT_NAME));
        String name = oadsPlatform.getName(); // getElementText(null, VESSEL_NAME_ELEMENT_NAME);
        platform.setPlatformName(name);
        PlatformType type = PlatformType.parse(oadsPlatform.getType()); // getElementText(null, PLATFORM_TYPE_ELEMENT_NAME));
        if ( PlatformType.UNKNOWN.equals(type) )
            type = DocumentHandler.guessPlatformType(name, datasetId);
        platform.setPlatformType(type);
        platform.setPlatformOwner(oadsPlatform.getOwner()); // getElementText(null, VESSEL_OWNER_ELEMENT_NAME));
        platform.setPlatformCountry(oadsPlatform.getCountry()); // getElementText(null, VESSEL_COUNTRY_ELEMENT_NAME));
        return platform;
    }
    ...
    private Coverage getCoverage() {
        ...
        coverage.setWesternLongitude(
                DocumentHandler.getNumericString(String.valueOf(_oadsMd.getWesternLongitude()), Coverage.LONGITUDE_UNITS)); // getElementText(null, WEST_BOUND_ELEMENT_NAME), Coverage.LONGITUDE_UNITS));
        coverage.setEasternLongitude(
                DocumentHandler.getNumericString(String.valueOf(_oadsMd.getEasternLongitude()), Coverage.LONGITUDE_UNITS)); // getElementText(null, EAST_BOUND_ELEMENT_NAME), Coverage.LONGITUDE_UNITS));
        coverage.setSouthernLatitude(
                DocumentHandler.getNumericString(String.valueOf(_oadsMd.getSouthernLatitude()), Coverage.LATITUDE_UNITS)); // getElementText(null, SOUTH_BOUND_ELEMENT_NAME), Coverage.LATITUDE_UNITS));
        coverage.setNorthernLatitude(
                DocumentHandler.getNumericString(String.valueOf(_oadsMd.getNorthernLatitude()), Coverage.LATITUDE_UNITS)); // getElementText(null, NORTH_BOUND_ELEMENT_NAME), Coverage.LATITUDE_UNITS));

        // CDIAC only has date stamps - use earliest and latest time of those days; should be reset from data
        Datestamp timestamp = Datestamp.fromUTC(new Date((long)(_oadsMd.getDataStartTime().doubleValue()*1000))); // getElementText(null, TEMP_START_DATE_ELEMENT_NAME));
        if ( timestamp != null ) {
            coverage.setEarliestDataTime(timestamp.getEarliestTime());
        }
        timestamp = Datestamp.fromUTC(new Date((long)(_oadsMd.getDataEndTime().doubleValue()*1000)));// getDatestamp(getElementText(null, TEMP_END_DATE_ELEMENT_NAME));
        if ( timestamp != null ) {
...
//        for (Element varElem : getElementList(null, VARIABLE_ELEMENT_NAME)) {
        for (BaseVariableType baseVar : _oadsMd.getOadsDocument().getVariables()) {
            Variable var = new Variable();
            String colName = baseVar.getDatasetVarName(); // (varElem, VARIABLES_NAME_ELEMENT_NAME);
            var.setColName(colName);
            var.setFullName(baseVar.getFullName()); // (varElem, VARIABLES_DESCRIPTION_ELEMENT_NAME));
            var.setVarUnit(baseVar.getUnits()); // getElementText(varElem, VARIABLES_UNIT_OF_VARIABLE_ELEMENT_NAME));

    // XXX  A lot of XXX stuff would have been put in Detailed Sampling and Analyzing Information (addlInfo).
            ...
            switch ( type ) {
                ...
                case XCO2_WATER_SST: {
                    if ( ! (baseVar instanceof Co2Socat)) {
                        throw new IllegalStateException("baseVar is not Co2Socat: " + baseVar);
                    }
                    Co2Socat co2Var = (Co2Socat)baseVar;
                    co2WaterVarIndices.add(k);
                    AquGasConc co2Water = new AquGasConc(var);
                    ArrayList<String> addnInfo = new ArrayList<String>();
                    if ( type == FCO2_WATER_EQU ||
                         type == PCO2_WATER_EQU ||   
                         type == XCO2_WATER_EQU) {
                        co2Water.setReportTemperature("equilibrator temperature");
                    } else {
                        co2Water.setReportTemperature("SST"); 
                    }
                    co2Water.setMeasureMethod(MethodType.MEASURED_INSITU);
                    co2Water.setInstrumentNames(Arrays.asList("Equilibrator", "CO2 Sensor"));
                    if ( PlatformType.MOORING.equals(platformType) )
                        co2Water.setObserveType("Time Series");
                    else
                        co2Water.setObserveType("Surface Underway");
                    String strVal = co2Var.getUncertainty(); // CO2_WATER_UNC_ELEMENT_NAME
                    NumericString numStr = DocumentHandler.getNumericString(strVal, null);
                    if ( numStr.isValid() )
                        co2Water.setAccuracy(numStr);
                    else
                        addnInfo.add("Accuracy/Uncertainty: " + strVal);
                    strVal = co2Var.getGasDetector().getResolution(); // CO2_WATER_RES_ELEMENT_NAME
                    numStr = DocumentHandler.getNumericString(strVal, null); 
                    if ( numStr.isValid() )
                        co2Water.setPrecision(numStr);
                    else
                        addnInfo.add("Precision/Resolution: " + strVal);
                    strVal = co2Var.getStandardization().getFrequency(); // CO2_FREQUENCY_ELEMENT_NAME);
                    if ( !(strVal == null || strVal.isEmpty() ))
                        addnInfo.add("Frequency: " + strVal);
                    co2Water.setMethodReference(co2Var.getMethodReference()); // METHOD_REFS_ELEMENT_NAME));
                    if ( co2Var.getCalculationMethod() != null && co2Var.getCalculationMethod().getDescription() != null ) {
                        co2Water.setMethodDescription(co2Var.getCalculationMethod().getDescription());
                    } else {
                        if ( co2Var.getDatasetVarName().toLowerCase().contains("fco2")) {
                            if ( co2Var.getCalculationMethodForFCO2() != null ) {
                                 co2Water.setMethodDescription(co2Var.getCalculationMethodForFCO2());
                            }
                        } else if ( co2Var.getCalculationMethodForPCO2() != null ) {
                             co2Water.setMethodDescription(co2Var.getCalculationMethodForPCO2());
                        }
                    }
                    co2Water.setSamplingLocation(co2Var.getLocationSeawaterIntake()); // INTAKE_LOCATION_ELEMENT_NAME));
                    co2Water.setSamplingElevation(
                            "Sampling Depth: " + co2Var.getDepthSeawaterIntake()); // INTAKE_DEPTH_ELEMENT_NAME));
                    co2Water.setDryingMethod(co2Var.getCo2GasDryingMethod()); // DRYING_METHOD_ELEMENT_NAME));
                    // XXX  Not clear which OADS element this would be.
//                    strVal = getElementText(null, DETAILS_OF_CO2_SENSING_ELEMENT_NAME);
//                    if ( !strVal.isEmpty() )
//                        addnInfo.add("Details of CO2 Sensing: " + strVal);
                    co2Water.setAddnInfo(addnInfo);
                    var = co2Water;
                    break;
                }
                // The only differences here are the name of the variable (not the type)
                // and the ReportTemperature.
//                case FCO2_WATER_SST:
//                case PCO2_WATER_SST:
//                case XCO2_WATER_SST: {
//                    co2WaterVarIndices.add(k);
...
//                }
                ...
                case XCO2_ATM_INTERP: {
                    ...
                    GasConc co2Atm = new GasConc(var); // <<<
                    ArrayList<String> addnInfo = new ArrayList<String>();
                    if ( type == FCO2_ATM_ACTUAL ||
                         type == PCO2_ATM_ACTUAL ||
                         type == XCO2_ATM_ACTUAL ) {
                        co2Atm.setMeasureMethod(MethodType.MEASURED_INSITU);
                    } else {
                        co2Atm.setMeasureMethod(MethodType.COMPUTED); // <<<<
                    }
                    co2Atm.setInstrumentNames(Collections.singletonList("CO2 Sensor")); 
                    if ( PlatformType.MOORING.equals(platformType) )
                        co2Atm.setObserveType("Time Series");
                    else
                        co2Atm.setObserveType("Surface Underway");
                    String strVal = co2Var.getUncertainty(); // CO2_AIR_UNC_ELEMENT_NAME); 
                    NumericString numStr = DocumentHandler.getNumericString(strVal, null);
                    if ( numStr.isValid() )
                        co2Atm.setAccuracy(numStr);
                    else
                        addnInfo.add("Accuracy/Uncertainty: " + strVal);
                    GasDetectorType gassy = co2Var.getGasDetector();
                    if ( gassy != null ) {
                        strVal = gassy.getResolution(); // CO2_AIR_RES_ELEMENT_NAME); 
                        numStr = DocumentHandler.getNumericString(strVal, null);
                        if ( numStr.isValid() )
                            co2Atm.setPrecision(numStr);
                        else
                            addnInfo.add("Precision/Resolution: " + strVal);
                    }
                    co2Atm.setMethodReference(co2Var.getMethodReference()); // METHOD_REFS_ELEMENT_NAME));
                    if ( co2Var.getCalculationMethod() != null ) {
                        co2Atm.setMethodDescription(co2Var.getCalculationMethod().getDescription());
                    }
//                    co2Atm.setMethodDescription(co2Var.getMethodReference());
//                    co2Atm.setMethodDescription(getElementText(null, CO2_MEASUREMENT_METHOD_ELEMENT_NAME)); // XXX See above
// XXX                   co2AtmActual.setSamplingLocation(co2Var.getLocationSeawaterIntake()); // XXX ??? MARINE_AIR_LOCATION_ELEMENT_NAME));
                    co2Atm.setDryingMethod(co2Var.getCo2GasDryingMethod()); // XXX ??? MARINE_AIR_DRYING_ELEMENT_NAME));
                    // XXX
//                    strVal = getElementText(null, DETAILS_OF_CO2_SENSING_ELEMENT_NAME);
//                    if ( !strVal.isEmpty() )
//                        addnInfo.add("Details of CO2 Sensing: " + strVal);
...
//                        addnInfo.add("Measurement: " + strVal);
                    co2Atm.setAddnInfo(addnInfo);
                    var = co2Atm;
                    break;
                }
                // XXX This is the same except for "computed" vs in-situ
//                case FCO2_ATM_INTERP:
//                case PCO2_ATM_INTERP:
//                case XCO2_ATM_INTERP: {
//                    if ( ! (baseVar instanceof Co2Socat)) {
...
//                }
                case SEA_SURFACE_TEMPERATURE: {
                    ...
                    String strVal = temp.getUncertainty(); // SST_ACCURACY_ELEMENT_NAME);
                    // XXX ??? Should have been sorted out in the CDIAC -> OCADS translation. 
//                    if ( strVal.isEmpty() )
...
//                        strVal = getElementText(null, SST_UNCERTAINTY_ELEMENT_NAME);
                    ...
                    if ( numStr.isValid() )
                        ...
                        addnInfo.add("Accuracy/Uncertainty: " + strVal);
                    // XXX  A lot of this stuff would be in additional Info.
...
//                    sst.setSamplingLocation(getElementText(null, SST_LOCATION_ELEMENT_NAME));
                ...
                case EQUILIBRATOR_TEMPERATURE: { // XXX ??? This needs to move into CO2 section
                    BaseVariableType temp = baseVar;
                    ...
                    String strVal = temp.getUncertainty(); // EQT_ACCURACY_ELEMENT_NAME);
//                    if ( strVal.isEmpty() )
...
//                    tequ.setSamplingLocation(getElementText(null, EQT_LOCATION_ELEMENT_NAME));
                ...
                case SEA_LEVEL_PRESSURE: {
                    BaseVariableType pres = (BaseVariableType)baseVar;
                    ...
                    String strVal = pres.getUncertainty(); // ATM_ACCURACY_ELEMENT_NAME);
//                    if ( strVal.isEmpty() )
...
//                        slp.setPressureCorrection("Normalized: " + strVal);
                ...
                case EQUILIBRATOR_PRESSURE: { // XXX ??? This needs to move into CO2 section
                    BaseVariableType equPres = (BaseVariableType)baseVar;
                    ...
                    String strVal = equPres.getUncertainty(); // (null, EQP_ACCURACY_ELEMENT_NAME);
//                    if ( strVal.isEmpty() )
...
//                        pequ.setPressureCorrection("Normalized: " + strVal);
                ...
                case SALINITY: {
                    BaseVariableType salt = (BaseVariableType)baseVar;
                    ...
                    String strVal = salt.getUncertainty(); // SSS_ACCURACY_ELEMENT_NAME);
//                    if ( strVal.isEmpty() )
...
//        0.95 L; Water Volume: 0.4 L; Gas Volume: 0.55 L
    private static final Pattern ALT_AOML_VOLUME_DESCRIPTION_PATTERN =
            Pattern.compile("(\\d*\\.?\\d*)\\s*L;\\s*" +
                    "Water Volume:\\s*(\\d*\\.?\\d*)\\s*L\\s*;\\s*" +
                    "Gas Volume:\\s*(\\d*\\.?\\d*)\\s*L\\s*"
            );
    /**
    ...
    List<VarType>EQU_VARS = Arrays.asList(new VarType[] {
            FCO2_WATER_EQU,
            FCO2_WATER_SST,
            PCO2_WATER_EQU,
            PCO2_WATER_SST,
    ...
    private ArrayList<Instrument> getInstruments() {
        ...
        List<Co2Socat> co2Vars = getCo2Vars();
        Co2Socat equVar = null;
        for (Co2Socat co2var : co2Vars) {
            VarType type = getVarTypeFromColumnName(co2var.getDatasetVarName());
            if ( EQU_VARS.contains(type)) {
                equVar = co2var;
                break;
            }
        }
        if  ( equVar == null ) {
            throw new IllegalStateException("No Equilibrator data var found.");
        }
        
        EquilibratorType equ = equVar.getEquilibrator();
        ...
        equilibrator.setEquilibratorType(equ.getType()); //  EQUI_TYPE_ELEMENT_NAME));
        String volumeDesc = equ.getVolume(); // EQUI_VOLUME_ELEMENT_NAME);
        Matcher matcher = AOML_VOLUME_DESCRIPTION_PATTERN.matcher(volumeDesc);
        if ( !matcher.matches() ) {
            matcher = ALT_AOML_VOLUME_DESCRIPTION_PATTERN.matcher(volumeDesc);
        }
        if ( matcher.matches()) {
        ...
        equilibrator.setWaterFlowRate(equ.getWaterFlowRate()); //  WATER_FLOW_RATE_ELEMENT_NAME));
        equilibrator.setGasFlowRate(equ.getGasFlowRate()); // GAS_FLOW_RATE_ELEMENT_NAME));
        equilibrator.setVenting(equ.getVented()); //  VENTED_ELEMENT_NAME));
        // XXX This is currently showing up in OADS:<samplingInstrument/> ...
//        equilibrator.setAddnInfo(getListOfLines(getElementText(null, EQUI_ADDITIONAL_INFO_ELEMENT_NAME)));
        ...
        GasSensor co2Sensor = null;
        GasDetectorType gas = equVar.getGasDetector();
        if ( gas != null ) {
            co2Sensor = new GasSensor();
            co2Sensor.setName("CO2 Sensor");
            co2Sensor.setManufacturer(gas.getManufacturer()); //  CO2_SENSOR_MANUFACTURER_ELEMENT_NAME));
            co2Sensor.setModel(gas.getModel()); //  CO2_SENSOR_MODEL_ELEMENT_NAME));
            // add in standardization.getFrequency() ?
            StandardizationType std = equVar.getStandardization();
            if ( std != null ) {
                co2Sensor.setCalibration(std.getDescription()); // CO2_SENSOR_CALIBRATION_ELEMENT_NAME));
                // continues below
...
//        }
         // =================================
        List<StandardGasType> stdGases = std.getStandardGas();
        ArrayList<CalibrationGas> gasList = new ArrayList<CalibrationGas>(stdGases.size());
        int num = 0;
        for (StandardGasType stdGas : stdGases) {
            num += 1;
            String id = "N/A";
            String supplier = stdGas.getManufacturer();
            if ( supplier.indexOf("from") >= 0 ) {
                String[] parts = supplier.split("from");
                id = parts[0].trim();
                supplier = parts[1].trim();
            }
            String concStr = stdGas.getConcentration();
            String accStr = stdGas.getUncertainty();
            String useFreq = getFrequency(std.getFrequency(), num, id);
            List<String[]> concs = new ArrayList<>();
            String sep = null;
            if ( concStr != null ) {
                if ( concStr.contains(";")) { sep = ";"; }
                else if ( concStr.contains(",")) { sep = ","; }
                if ( sep != null ) {
                    String[] parts = concStr.trim().split(sep);
                    String sep2 = null;
                    for ( String part : parts ) {
                        if ( part.contains(",")) { sep2 = ","; }
                        else if ( part.contains("=") ) { sep2 = "="; }
                        if ( sep2 != null ) {
                            String[] parts2 = part.split(sep2);
                            if ( parts2.length == 2 ) {
                                concs.add(parts2);
                            } else {
                    ...
                    for (String[] conky : concs) {
                        try {
                            gasList.add(new CalibrationGas(conky[0], "CO2", supplier, conky[1], accStr, useFreq));
                        } catch (Exception ex) {
                            System.err.println("Failed to add calibration gas : " + ex.getMessage());
                            System.err.println(" - supplier: " + supplier);
                            System.err.println(" - concentration: " + concStr);
                            System.err.println(" - accuracy: " + accStr);
                            ex.printStackTrace();
                        }
                    }
                } else {
                ...
                System.out.println("No calibration gases found."); // XXX Somehow need to trap these messages.
                if ( accStr != null || useFreq != null ) {
                    System.out.println("WARNING: accuracy string " + accStr + 
                                       ", or useFreq " + useFreq + " found without concentration!");
                }
            }
        }
        /*
     ...
     * @return
     */
    private String getFrequency(String freqString, int num, String id) {
        String freq = "N/A";
        if ( ! StringUtils.emptyOrNull(freqString)) {
            if ( freqString.indexOf(num+":") >= 0 ) {
                int start = freqString.indexOf(num+":")+2;
                int end = freqString.indexOf(',', start);
                freq = end > 0 ?
                        freqString.substring(start, end) :
                        freqString.substring(start);
            } else if ( freqString.indexOf(id+":") >= 0 ) {
                int start = freqString.indexOf(id+":")+id.length()+1;
                int end = freqString.indexOf(',', start);
                freq = end > 0 ?
                        freqString.substring(start, end) :
                        freqString.substring(start);
     ...
     * @return
     */
    private List<Co2Socat> getCo2Vars() {
        List<Co2Socat> co2Vars = new ArrayList<>();
        for (BaseVariableType baseVar : _oadsMd.getOadsDocument().getVariables()) {
            if ( baseVar instanceof Co2Socat ) {
//                System.out.println("Found co2var: " + baseVar);
                co2Vars.add(((Co2Socat)baseVar));
            }
        }
        return co2Vars;
    }
