...
public class OadsUtils {
    ...
    public static SocatMetadata createSdiMetadataFromOadsMetadata() {
        return null;
    }
     ...
     */
    public static DatasetQCStatus _suggestDatasetQCFlag(OadsMetadataDocumentType oadsDoc,
                                                       SocatMetadata sdiMData,
                                                       DashboardDataset dataset , CdiacReader cdr) 
        throws IllegalArgumentException 
    {
        HashSet<String> varColNames = new HashSet<String>();
        HashMap<String,GasSensor> co2SensorsMap = new HashMap<String,GasSensor>();
        for (BaseVariableType var : oadsDoc.getVariables()) {
            String varName = var.getDatasetVarName();
            if ( !varColNames.add(varName)) {
                throw new IllegalArgumentException(
                        "A column name is used to describe multiple columns the OME metadata");
            }
            VarType vt = cdr.getVarTypeFromColumnName(varName);
            System.out.println(varName + " VarType:"+vt);
//            String co2uncertainty = "Unknown";
            if ( var instanceof Co2Socat ) {
                Co2Socat co2 = (Co2Socat) var;
                System.out.println(var);
                StandardizationType std = co2.getStandardization();
                System.out.println(varName + " std " + std);
//                List<StandardGasType> stdGases = std.getStandardGas();
//                GasDetectorType co2detector = co2.getGasDetector();
                String uncertainty = co2.getUncertainty();
                System.out.println(varName + " Uncertainty:"+uncertainty);
                GasSensor co2sensor = new GasSensor();
                co2sensor.setName(co2.getSamplingInstrument());
                if ( co2SensorsMap.put(co2.getName(), co2sensor) != null) {
                            throw new IllegalArgumentException("Unexpected duplicate name '" +
                                    co2sensor.getName() + "' for a CO2 sensor");
                }
//                break;
            }
        }
...
//                        "A column name is used to describe multiple columns the OME metadata");
    ...
    public static DatasetQCStatus suggestDatasetQCFlag(// OadsMetadataDocumentType oadsDoc,
                                                       SocatMetadata sdiMData,
                                                       DashboardDataset dataset)  // , CdiacReader cdr) 
        throws IllegalArgumentException 
    {
